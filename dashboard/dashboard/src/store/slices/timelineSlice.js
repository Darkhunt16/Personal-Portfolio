...
export const getAllTimeline = () => async (dispatch) => {
  ...
  try {
    const {data} = await axios.get(
      "https://personal-portfolio-25a9.onrender.com/api/v1/timeline/getall",
...
export const addNewTimeline = (timelineData) => async (dispatch) => {
  ...
  try {
    const {data} = await axios.post(
      "https://personal-portfolio-25a9.onrender.com/api/v1/timeline/add",
      timelineData,
      {
        withCredentials: true,
    ...
    dispatch(
      timelineSlice.actions.addNewTimelineSuccess(data.message)
    );
    dispatch(timelineSlice.actions.clearAllErrors());
  } catch (error) {
    dispatch(
...
export const deleteTimeline = (id) => async (dispatch) => {
  ...
  try {
    const {data} = await axios.delete(
      `https://personal-portfolio-25a9.onrender.com/api/v1/timeline/delete/${id}`,
      {
        withCredentials: true,
      }
    );
    dispatch(
      timelineSlice.actions.deleteTimelineSuccess(data.message)
    );
    dispatch(timelineSlice.actions.clearAllErrors());
  } catch (error) {
    dispatch(
