    ...
    <div className="w-full flex flex-col gap-8 sm:gap-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {apps &&
          apps.map((element) => {
            return ( 
              <Card className="h-40 sm:h-50 md:h-64 lg:h-60 p-4 flex flex-col justify-center items-center gap-3 overflow-hidden" key={element._id}>
  <img
    src={element.svg && element.svg.url}
    alt="skill"
    className="h-12 sm:h-24 w-auto"
  />
  <p className="text-muted-foreground text-center truncate">
    {element.name}
  </p>
</Card>
            );
